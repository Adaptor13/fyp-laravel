...
@section('main-content')
    <div class="container-fluid">
        <div class="row">
            <div class="col-sm-6">
                <h4 class="main-title">Law Enforcement Management</h4>
            </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body p-0">
                        <div class="overflow-auto data-table-style app-scroll">
                            <table id="lawEnforcementTable" class="display app-data-table deafult-data-tabel">
                                <thead>
                                    <tr>
                                        <th>Name</th>
                                        <th>Agency</th>
    ...
    <div class="modal fade" id="addLawEnforcement" aria-hidden="true" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                <form method="POST" action="{{ route('users.law.store') }}" enctype="multipart/form-data">
                    @csrf

                    <div class="modal-header bg-primary">
                        <h5 class="modal-title text-white">Add Law Enforcement</h5>
                    ...
                    <div class="modal-body">
                                ...
                                <div class="d-flex justify-content-center mb-3">
                                    <div class="avatar-upload">
                                        <div class="avatar-edit">
                                            <input type="file" id="imageUploadAddLE" name="avatar"
                                                accept=".png,.jpg,.jpeg" data-preview="#imgPreviewAddLE">
                                            <label for="imageUploadAddLE"><i class="ti ti-photo-heart"></i></label>
                                        </div>
                                        <div class="avatar-preview">
                                            <div id="imgPreviewAddLE"></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select id="add_agency" name="agency" class="form-select" required>
                                    ...
                                    <option value="AADK" {{ old('agency') === 'AADK' ? 'selected' : '' }}>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label for="add_rank" class="form-label">Rank</label>
                                <input id="add_rank" name="rank" type="text" class="form-control"
                                    value="{{ old('rank') }}" placeholder="e.g. Inspector">
                            </div>

                            <div class="col-md-6 mb-3">
                                <label for="add_station" class="form-label">Station</label>
                                <input id="add_station" name="station" type="text" class="form-control"
                                    value="{{ old('station') }}" placeholder="e.g. IPD Shah Alam">
                            </div>
                        </div>
    ...
    <div class="modal fade" id="editLawEnforcement" aria-hidden="true" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                <form method="POST" action="{{ route('users.law.update', '__ID__') }}"
                    data-action-template="{{ route('users.law.update', '__ID__') }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')

                    <div class="modal-header bg-primary">
                        <h5 class="modal-title text-white">Edit Law Enforcement</h5>
                        <button type="button" class="btn-close m-0 fs-5" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                    </div>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="d-flex justify-content-center mb-3">
                                    <div class="avatar-upload">
                                        <div class="avatar-edit">
                                            <input type="file" id="imageUploadEditLE" name="avatar"
                                                accept=".png,.jpg,.jpeg" data-preview="#imgPreviewEditLE">
                                            <label for="imageUploadEditLE"><i class="ti ti-photo-heart"></i></label>
                                        </div>
                                        <div class="avatar-preview">
                                            <div id="imgPreviewEditLE"
                                                style="background-image:url('{{ $lawEnforcement?->avatar_url ?? '' }}');">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select id="edit_agency" name="agency" class="form-select" required>
                                    ...
                                    <option value="AADK">Agensi Antidadah Kebangsaan (AADK)</option>
                                </select>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input id="edit_rank" name="rank" type="text" class="form-control"
                                    placeholder="e.g. Inspector">
                            </div>

                            <div class="col-md-6 mb-3">
                                <label for="edit_station" class="form-label">Station</label>
                                <input id="edit_station" name="station" type="text" class="form-control"
                                    placeholder="e.g. IPD Shah Alam">
                            </div>
                        </div>
...
@section('script')
    ...
    <script>
        ...
        $(function() {
            $('#lawEnforcementTable').DataTable({
                processing: true,
                ajax: {
                    url: '{{ route('users.law.data') }}',
                    type: 'GET',
                    dataSrc: 'data'
                },
                columns: [{
                                ...
                                actionButtons += `
                                    <li>
                                        <button type="button" class="dropdown-item edit-btn"
                                                ...
                                                data-bs-target="#editLawEnforcement">
        ...
        $('#addLawEnforcement, #editLawEnforcement').on('show.bs.modal', function() {
            $('#add_phone, #edit_phone').mask('000-0000000');
        });

        $(document).on('click', '#lawEnforcementTable .edit-btn', function() {
            const table = $('#lawEnforcementTable').DataTable();
            const rowData = table.row($(this).closest('tr')).data();
            if (!rowData) return;

            const $modal = $('#editLawEnforcement');
            ...
            // Law enforcement profile fields
            $modal.find('select[name="agency"]').val(rowData.agency ?? '').trigger('change');
            $modal.find('input[name="badge_number"]').val(rowData.badge_number ?? '');
            $modal.find('input[name="rank"]').val(rowData.rank ?? '');
            $modal.find('input[name="station"]').val(rowData.station ?? '');
            $modal.find('input[name="le_state"]').val(rowData.le_state ?? '');

            // Contact info (from user_profiles in your JSON)
            ...
            // Avatar preview (supports LE-specific or fallback id)
            const avatarUrl = rowData.avatar_url || rowData.profile?.avatar_url || '';
            const $preview = $modal.find('#imgPreviewEditLE').length ?
                $modal.find('#imgPreviewEditLE') :
