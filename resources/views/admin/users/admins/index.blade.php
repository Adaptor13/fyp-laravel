...
@section('main-content')
                ...
                <div class="card">
                    ...
                    <div class="card-body p-0">
                        <div class="overflow-auto data-table-style app-scroll">
                            <table id="adminTable" class="display app-data-table deafult-data-tabel">
                                <thead>
                                    <tr>
                                        <th>Name</th>
                                        <th>Department</th>
    ...
    <div class="modal fade" id="addAdmin" aria-hidden="true" tabindex="-1">
                                ...
                                <div class="d-flex justify-content-center mb-3">
                                    <div class="avatar-upload">
                                        <div class="avatar-edit">
                                            <input type="file" id="imageUploadAddAdmin" name="avatar"
                                                accept=".png,.jpg,.jpeg" data-preview="#imgPreviewAddAdmin">
                                            <label for="imageUploadAddAdmin"><i class="ti ti-photo-heart"></i></label>
                                        </div>
                                        <div class="avatar-preview">
                                            <div id="imgPreviewAddAdmin"></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <button class="btn btn-outline-secondary toggle-password" type="button" data-target="#add_admin_password">
                                        <i class="ti ti-eye"></i>
                                    </button>
                                </div>
                            </div>
                            <div class="col-md-6 mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <button class="btn btn-outline-secondary toggle-password" type="button"
                                            data-target="#add_admin_password_confirmation">
                                        <i class="ti ti-eye"></i>
                                    </button>
                                </div>
                            </div>
    ...
    <div class="modal fade" id="editAdmin" aria-hidden="true" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                <form method="POST"
                    action="{{ route('users.admin.update', ['user' => '__ID__']) }}"
                    data-action-template="{{ route('users.admin.update', ['user' => '__ID__']) }}"
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="d-flex justify-content-center mb-3">
                                    <div class="avatar-upload">
                                        <div class="avatar-edit">
                                            <input type="file" id="imageUploadEditAdmin" name="avatar"
                                                accept=".png,.jpg,.jpeg" data-preview="#imgPreviewEditAdmin">
                                            <label for="imageUploadEditAdmin"><i class="ti ti-photo-heart"></i></label>
                                        </div>
                                        <div class="avatar-preview">
                                            <div id="imgPreviewEditAdmin"></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <form id="deleteForm" method="POST" action="{{ route('users.admin.destroy', '__id__') }}" class="d-none">
        @csrf
        @method('DELETE')
    </form>
    ...
    <script>
        ...
        $(function() {
            $('#adminTable').DataTable({
                processing: true,
                ajax: {
                    url: '{{ route('users.admin.data') }}', // <-- updated route
                    type: 'GET',
                    dataSrc: 'data'
                },
                columns: [
            ...
            $('#confirmDeleteBtn').on('click', function() {
                ...
                form.action = "{{ route('users.admin.destroy', '__id__') }}".replace('__id__', deleteId);
                form.submit();
            });
        });


        $('#addAdmin, #editAdmin').on('show.bs.modal', function() {
            $('#add_phone, #edit_phone').mask('000-0000000');
        });

       $(document).on('click', '#adminTable .edit-btn', function () {
        ...
        // Modal + form
        const $modal = $('#editAdmin');                
        const $form  = $modal.find('form');

        // Replace __ID__ in action template once per open
        ...
        // Admin profile
        $modal.find('input[name="department"]').val(rowData.department ?? '');
        $modal.find('input[name="position"]').val(rowData.position ?? '');
        ...
        const $preview = $modal.find('#imgPreviewEditAdmin');   // <-- updated preview id
